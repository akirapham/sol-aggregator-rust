...
use crate::pool_data_types::clmm::tick::{TickUtils, TICK_ARRAY_BITMAP_SIZE, TICK_ARRAY_SIZE};
use crate::pool_data_types::clmm::tick_query::TickQuery;
use crate::pool_data_types::clmm::util::{
    is_zero, leading_zeros, least_significant_bit, most_significant_bit, trailing_zeros,
};
use crate::pool_data_types::raydium_clmm::TickArrayBitmapExtension;
use rug::Complete;
...
impl TickArrayBitmap {
    pub fn get_bitmap_tick_boundary(
        ...
        if tick_array_start_index < 0 {
            ...
            GetBitmapTickBoundary {
                min: min_neg,
                max: min_neg + ticks_in_one_bitmap,
            }
        } else {
            GetBitmapTickBoundary {
                min,
                max: min + ticks_in_one_bitmap,
            }
        }
    }
    ...
    pub fn next_initialized_tick_array_start_index(
        ...
        {
            return Ok(NextTickArray {
                is_init: false,
                tick_index: last_tick_array_start_index,
            });
        }
        ...
        if zero_for_one {
            ...
            match least_significant_bit(1024, &offset_bitmap) {
                None => Ok(NextTickArray {
                    is_init: false,
                    tick_index: tick_boundary - TickQuery::tick_count(tick_spacing) as i32,
                }),
                Some(next_bit) => {
                    let next_array_start_index = (bit_pos + next_bit as i32 - 512) * multiplier;
                    Ok(NextTickArray {
...
impl TickArrayBitmapExtensionUtils {
    pub fn next_initialized_tick_array_in_bitmap(
        ...
        let bitmap_tick_boundary =
            TickArrayBitmap::get_bitmap_tick_boundary(next_tick_array_start_index, tick_spacing);
        ...
        if zero_for_one {
            ...
            if is_zero(512, &offset_bitmap) {
                ...
                let next_array_start_index = next_tick_array_start_index
                    - next_bit as i32 * TickQuery::tick_count(tick_spacing) as i32;
                NextTickArray {
                    is_init: true,
                    tick_index: next_array_start_index,
                }
    ...
    pub fn extension_tick_boundary(
        tick_spacing: u16,
    ) -> Result<ExtensionTickBoundaryResult, String> {
        let positive_tick_boundary =
        ...
        if MAX_TICK <= positive_tick_boundary {
            return Err(format!(
                "extensionTickBoundary check error: {MAX_TICK}, {positive_tick_boundary}"
            ));
        }
        if negative_tick_boundary <= MIN_TICK {
            return Err(format!(
                "extensionTickBoundary check error: {negative_tick_boundary}, {MIN_TICK}"
            ));
        }
        Ok(ExtensionTickBoundaryResult {
            positive_tick_boundary,
            negative_tick_boundary,
        })
    }
    pub fn check_extension_boundary(tick_index: i32, tick_spacing: u16) -> Result<(), String> {
        ...
        if tick_index >= extension_tick_boundary.negative_tick_boundary
            && tick_index < extension_tick_boundary.positive_tick_boundary
        {
    ...
    pub fn check_tick_array_is_init(
        ...
        let is_initialized =
            TickUtils::merge_tick_array_bitmap(&tick_array_bitmap.tick_array_bitmap)
                .get_bit(tick_array_offset_in_bitmap);
        Ok(CheckTickArrayIsInitResult {
            is_initialized,
            start_index: tick_array_start_index,
        })
    }
    pub fn tick_array_offset_in_bitmap(tick_array_start_index: i32, tick_spacing: u16) -> u32 {
        ...
        if tick_array_start_index < 0 && m != 0 {
            tick_array_offset_in_bitmap =
                TICK_ARRAY_BITMAP_SIZE as u32 - tick_array_offset_in_bitmap;
        }
        tick_array_offset_in_bitmap
    }
}
