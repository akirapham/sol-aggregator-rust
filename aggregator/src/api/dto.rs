...
use serde::{Deserialize, Serialize}; use solana_sdk::pubkey::Pubkey;
// Ensure serde is imported for Json serialization
use validator::{Validate, ValidationError};
use axum::{http::StatusCode, Json};

use crate::{aggregator::DexAggregator, types::{SwapStep, Token}};
...
pub struct QuoteRequest {
    #[validate(length(min = 32, max = 44, message = "Input token must be a valid Solana public key (32-44 characters)"))]
    pub input_token: String,

    #[validate(length(min = 32, max = 44, message = "Output token must be a valid Solana public key (32-44 characters)"))]
    pub output_token: String,

    #[validate(length(min = 32, max = 44, message = "User wallet must be a valid Solana public key (32-44 characters)"))]
    pub user_wallet: String,

    #[validate(range(min = 1, message = "Input amount must be greater than 0"))]
    pub input_amount: u64,

    #[validate(range(min = 0, max = 10000, message = "Slippage must be between 0 and 10000 basis points (0-100%)"))]
...
pub fn parse_pubkey_with_error(pubkey_str: &str, field_name: &str) -> Result<Pubkey, (StatusCode, Json<ErrorResponse>)> {
    ...
    match aggregator.get_pool_manager().get_token(pubkey).await {
        ...
        None => Err((
            ...
            Json(ErrorResponse {
                ...
                details: vec![format!("{} token '{}' not found in pool manager", token_type, token_str)],
            }),
        )),
    }
}
