use crate::pool_data_types::clmm::constants::{MAX_TICK, MIN_TICK};
use crate::pool_data_types::clmm::math::SwapMath;
use crate::pool_data_types::clmm::pda::get_pda_tick_array_address;
use crate::pool_data_types::clmm::tick::TickUtils;
use crate::pool_data_types::clmm::tick_array_bitmap::{
    TickArrayBitmap, TickArrayBitmapExtensionUtils,
};
use crate::pool_data_types::clmm::tick_query::TickQuery;
...
impl PoolUtils {
    pub fn is_overflow_default_tickarray_bitmap(
        tick_spacing: u16,
        tick_array_start_indexs: Vec<i32>,
    ) -> bool {
        let tick_range = Self::tick_range(tick_spacing);
        for tick_index in tick_array_start_indexs {
            let tick_array_start_index =
                TickUtils::get_tick_array_start_index_by_tick(tick_index, tick_spacing);
            if tick_array_start_index >= tick_range.max_tick_boundary
                || tick_array_start_index < tick_range.min_tick_boundary
            {
                return true;
            }
        }
    ...
    pub fn tick_range(tick_spacing: u16) -> TickRange {
        let mut max_tick_boundary =
            TickArrayBitmap::max_tick_in_tick_array_bitmap(tick_spacing) as i32;
        let mut min_tick_boundary = max_tick_boundary.neg();
        if max_tick_boundary > MAX_TICK {
            max_tick_boundary = TickQuery::get_array_start_index(MAX_TICK, tick_spacing)
                + TickQuery::tick_count(tick_spacing) as i32;
        }
    ...
    pub fn next_initialized_tick_array_start_index(
        ...
        let mut last_tick_array_start_index =
            TickQuery::get_array_start_index(tick_current, tick_spacing);
        loop {
            let next_initialized_tick_array_start_index =
                TickArrayBitmap::next_initialized_tick_array_start_index(
            ...
            let next_initialized_tick_array_from_one_bitmap =
                TickArrayBitmapExtensionUtils::next_initialized_tick_array_from_one_bitmap(
                    last_tick_array_start_index,
                    tick_spacing,
                    zero_for_one,
                    ex_bitmap_info,
    ...
    pub fn get_first_initialized_tick_array(
        ...
        let array_start_index = if Self::is_overflow_default_tickarray_bitmap(
            pool_info.pool_state.tick_spacing,
            vec![pool_info.pool_state.tick_current_index],
        ) {
            // println!("1");
            TickArrayBitmapExtensionUtils::check_tick_array_is_init(
                TickQuery::get_array_start_index(
            ...
            TickUtils::check_tick_array_is_initialized(
                &TickUtils::merge_tick_array_bitmap(
                    pool_info.pool_state.tick_array_bitmap.as_slice(),
                ),
                pool_info.pool_state.tick_current_index,
                pool_info.pool_state.tick_spacing,
            )
        ...
        if array_start_index.is_initialized {
            let (next_account_meta, _) = get_pda_tick_array_address(
                &pool_info.program_id,
                &pool_info.id,
                &(array_start_index.start_index as i32),
            );
            Ok(GetFirstInitializedTickArrayResult {
                start_index: array_start_index.start_index,
                next_account_meta,
            })
        } else {
            match Self::next_initialized_tick_array_start_index(
                ...
                Some(next_init_start_index) => {
                    let (next_account_meta, _) = get_pda_tick_array_address(
                        &pool_info.program_id,
                        &pool_info.id,
                        &next_init_start_index,
                    );
                    Ok(GetFirstInitializedTickArrayResult {
                        start_index: next_init_start_index,
                        next_account_meta,
                    })
                }
            }
    ...
    pub fn get_output_amount_and_remain_accounts(
        pool_info: &ComputeClmmPoolInfo,
        ...
        let mut all_needed_accounts: Vec<Pubkey> =
            vec![first_tick_array_start_index.next_account_meta];
