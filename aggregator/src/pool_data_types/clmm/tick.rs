...
use crate::pool_data_types::clmm::tick_array_bitmap::CheckTickArrayIsInitResult;
use crate::pool_data_types::clmm::tick_query::TickQuery;
use crate::pool_data_types::{TickArrayBitmapExtension, TickArrayState, TickState};
use rug::Integer;
use std::ops::Shl;
use std::str::FromStr;
...
impl TickUtils {
    pub fn next_init_tick(
        ...
        let current_tick_array_start_index =
            TickQuery::get_array_start_index(current_tick_index, tick_spacing);
        if current_tick_array_start_index != tick_array_current.start_tick_index {
            return None;
        }
        let mut offset_in_array = ((current_tick_index - tick_array_current.start_tick_index)
            as f32
            / tick_spacing as f32)
            .floor() as usize;
        if zero_for_one {
            ...
            while offset_in_array > 0 {
                if tick_array_current.ticks[offset_in_array]
                    .liquidity_gross
                    .gt(&0)
                {
    ...
    pub fn first_initialized_tick(
        ...
        Err(format!(
            "firstInitializedTick check error: {} - {zero_for_one}",
    ...
    pub fn check_tick_array_is_initialized(
        ...
        CheckTickArrayIsInitResult {
            is_initialized: bitmap.get_bit(bit_pos as u32),
            start_index: (bit_pos - 512) * multiplier as i32,
        }
    }
    ...
    pub fn get_tick_array_start_index_by_tick(tick_index: i32, tick_spacing: u16) -> i32 {
        Self::get_tick_array_bit_index(tick_index, tick_spacing)
    ...
    pub fn get_initialized_tick_array_in_range(
        ...
        let tick_array_offset = (tick_array_start_index as f32
            / (tick_spacing as f32 * TICK_ARRAY_SIZE as f32))
            .floor() as i32;
    ...
    pub fn search_low_bit_from_start(
        ...
        let merged: Vec<Integer> = tick_array_bitmaps
            .iter()
            .map(|e| TickUtils::merge_tick_array_bitmap(e))
            .collect();
        let mut res: Vec<i32> = vec![];
        while current_tick_array_bit_start_index >= -7680 {
            let array_index =
                ((current_tick_array_bit_start_index + 7680) as f64 / 512_f64).floor() as usize;
