...
use crate::pool_data_types::clmm::tick::{TickUtils, TICK_ARRAY_SIZE};
...
impl TickQuery {
    ...
    pub fn check_is_valid_start_index(tick_index: i32, tick_spacing: u16) -> bool {
        if TickUtils::check_is_out_of_boundary(tick_index) {
            ...
            let min_start_index =
                TickUtils::get_tick_array_start_index_by_tick(MIN_TICK, tick_spacing);
    ...
    pub fn next_initialized_tick_array(
        ...
        let res = if zero_for_one {
            TickUtils::search_low_bit_from_start(
                tick_array_bitmap,
            ...
            TickUtils::search_high_bit_from_start(
                tick_array_bitmap,
                tick_array_bitmap_extension,
                current_offset + 1,
                1,
