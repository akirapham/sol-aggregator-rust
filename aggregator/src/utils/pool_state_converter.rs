...
use crate::utils::{get_sol_mint, tokens_equal, use_input_or_existing};
...
fn compute_pool_liquidity_usd(
    ...
    if !tokens_equal(token0, &sol_mint) {
        reserve0 as f64 / 1_000_000_000 as f64 * sol_price
    } else if !tokens_equal(token1, &sol_mint) {
        reserve1 as f64 / 1_000_000_000 as f64 * sol_price
    } else if is_base_token(&token0.to_string()) {
        reserve0 as f64 / 1_000_000 as f64
    } else if is_base_token(&token1.to_string()) {
        reserve1 as f64 / 1_000_000 as f64
    } else {
        0.0
    }
}
    ...
    match event {
        PoolUpdateEvent::PumpfunPoolUpdate(pumpfun_pool_update) => {
            Some(PoolState::PumpfunPoolState(PumpfunPoolState {
                ...
                liquidity_usd: pumpfun_pool_update.sol_reserve as f64 / 1_000_000_000 as f64
                    * sol_price,
