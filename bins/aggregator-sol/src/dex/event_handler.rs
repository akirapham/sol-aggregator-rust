...
use crate::{
    ...
    pool_data_types::{
        common, DbcPoolUpdate, MeteoraDammV2PoolUpdate, MeteoraDlmmPoolUpdate, PoolUpdateEventType,
        ...
        match_event!(event, {
                ...
                if let (Some(t0b), Some(t1b)) = (token0_balance, token1_balance) {
                    if is_base_token(&t0b.mint) || is_base_token(&t1b.mint) {
                        ...
                        let raydium_pool_update = RaydiumCpmmPoolUpdate {
                            ...
                            token0_vault: e.input_vault,
                            token1_vault: e.output_vault,
                ...
                if let (Some(t0b), Some(t1b)) = (token0_balance, token1_balance) {
                    if is_base_token(&t0b.mint) || is_base_token(&t1b.mint) {
                        let raydium_pool_update = RaydiumCpmmPoolUpdate {
                            ...
                            token0: Pubkey::from_str(&t0b.mint).unwrap(),
                            token1: Pubkey::from_str(&t1b.mint).unwrap(),
                            token0_vault: e.token0_vault,
                            token1_vault: e.token1_vault,
                            token0_reserve: t0b.amount,
                            token1_reserve: t1b.amount,
                ...
                if let (Some(btb), Some(qtb)) = (base_token_balance, quote_token_balance) {
                    if is_base_token(&btb.mint) || is_base_token(&qtb.mint) {
                        let raydium_pool_update = RaydiumAmmV4PoolUpdate {
                            ...
                            serum_program: Some(e.serum_program),
                            serum_market: Some(e.serum_market),
                            serum_bids: Some(e.serum_bids),
                            serum_asks: Some(e.serum_asks),
                            serum_event_queue: Some(e.serum_event_queue),
                            serum_coin_vault_account: Some(e.serum_coin_vault_account),
                            serum_pc_vault_account: Some(e.serum_pc_vault_account),
                            serum_vault_signer: Some(e.serum_vault_signer),
                            last_updated: e.metadata.recv_us as u64,
            ...
            BonkPoolStateAccountEvent => |e: BonkPoolStateAccountEvent| {
                pool_update_events.push(PoolUpdateEvent::Bonk(Box::new(
                    BonkPoolUpdate {
                        ...
                        global_config: e.pool_state.global_config,
                        platform_config: e.pool_state.platform_config,
                        base_mint: e.pool_state.base_mint,
                        quote_mint: e.pool_state.quote_mint,
                        base_vault: e.pool_state.base_vault,
                        quote_vault: e.pool_state.quote_vault,
                        creator: e.pool_state.creator,
                        last_updated: e.metadata.recv_us as u64,
            ...
            PumpSwapPoolAccountEvent => |e: PumpSwapPoolAccountEvent| {
                pool_update_events.push(PoolUpdateEvent::PumpSwap(
                    PumpSwapPoolUpdate {
                        ...
                        creator: Some(e.pool.creator),
                        base_mint: e.pool.base_mint,
                        quote_mint: e.pool.quote_mint,
                        pool_base_token_account: e.pool.pool_base_token_account,
                        pool_quote_token_account: e.pool.pool_quote_token_account,
                        last_updated: e.metadata.recv_us as u64,
                        base_reserve: 0,
                        quote_reserve: 0,
                        slot: e.metadata.slot,
                ...
                if is_base_token(&e.pool_state.token_mint0.to_string()) || is_base_token(&e.pool_state.token_mint1.to_string()) {
                    ...
                    Box::new(RaydiumClmmPoolUpdate {
                        ...
                        pool_state_part: Some(RaydiumClmmPoolStatePart {
                            amm_config: e.pool_state.amm_config,
                            token_mint0: e.pool_state.token_mint0,
                            token_mint1: e.pool_state.token_mint1,
                            token_vault0: e.pool_state.token_vault0,
                            token_vault1: e.pool_state.token_vault1,
                            observation_key: e.pool_state.observation_key,
            ...
            RaydiumCpmmPoolStateAccountEvent => |e: RaydiumCpmmPoolStateAccountEvent| {
                let raydium_pool_update = RaydiumCpmmPoolUpdate {
                    ...
                    slot: e.metadata.slot,
                    transaction_index: e.metadata.transaction_index,
                    token0: e.pool_state.token0_mint,
                    token1: e.pool_state.token1_mint,
                    token0_vault: e.pool_state.token0_vault,
                    token1_vault: e.pool_state.token1_vault,
                    token0_reserve: 0,
                    token1_reserve: 0,
                    amm_config: e.pool_state.amm_config,
                    observation_state: e.pool_state.observation_key,
                    last_updated: e.metadata.recv_us as u64,
