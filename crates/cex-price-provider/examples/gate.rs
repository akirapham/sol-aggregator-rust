...
async fn main() -> Result<()> {
    ...
    println!("Starting Gate.io Service Example...");
    println!("This example subscribes to tokens with valid contract addresses\n");

    // Create Gate.io service with Ethereum filter
    let gate_service = GateService::new(FilterAddressType::Ethereum);

    println!("Starting Gate.io WebSocket connections...");

    // Start the service in a background task
    let service_clone = std::sync::Arc::new(gate_service);
    let service_for_start = service_clone.clone();

    tokio::spawn(async move {
        if let Err(e) = service_for_start.start().await {
            eprintln!("Gate.io service error: {}", e);
        }
    });
