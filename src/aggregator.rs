...
use crate::types::{
    AggregatorConfig, BestRoute, DexType, ExecutionPriority, MevRisk, PriceInfo, RouteType,
...
impl DexAggregator {
    ...
    /// Get price comparison across all DEXs
    pub async fn get_price_comparison(
        ...
        for (dex_type, dex) in &self.dexs {
            if self.config.enabled_dexs.contains(dex_type) {
                let dex_clone = dex.as_ref();
                let input_token_clone = *input_token;
                ...
                let task = async move {
                    match timeout(
                        Duration::from_secs(5),
                        dex_clone.get_price(&input_token_clone, &output_token_clone, amount),
                    )
                    .await
                    {
                        Ok(Ok(price_info)) => Some(price_info),
                        Ok(Err(e)) => {
                            log::warn!("DEX {} price error: {}", dex_type_clone, e);
                            None
                        }
                        Err(_) => {
                            log::warn!("DEX {} price timeout", dex_type_clone);
                            None
                        }
                    }
                };
    ...
    /// Get all supported tokens across all DEXs
    pub async fn get_all_supported_tokens(&self) -> Result<Vec<crate::types::Token>> {
        ...
        for (dex_type, dex) in &self.dexs {
            if self.config.enabled_dexs.contains(dex_type) {
                ...
                let task = async move {
                    match timeout(Duration::from_secs(5), dex_clone.get_supported_tokens()).await {
                        Ok(Ok(tokens)) => Some((dex_type_clone, tokens)),
                        Ok(Err(e)) => {
                            log::warn!("DEX {} tokens error: {}", dex_type_clone, e);
                            None
                        }
                        Err(_) => {
                            log::warn!("DEX {} tokens timeout", dex_type_clone);
                            None
                        }
                    }
                };
        ...
        for result in results {
            if let Some((dex_type, tokens)) = result {
                for mut token in tokens {
                    // Add DEX information to token metadata
        ...
        for token in all_tokens {
            if seen_addresses.insert(token.address) {
    ...
    /// Check if a token pair is supported by any DEX
    pub async fn is_token_pair_supported(
        &self,
        token_a: &Pubkey,
        token_b: &Pubkey,
    ) -> Result<bool> {
        ...
        for (dex_type, dex) in &self.dexs {
            if self.config.enabled_dexs.contains(dex_type) {
                let dex_clone = dex.as_ref();
                let token_a_clone = *token_a;
                let token_b_clone = *token_b;
                let dex_type_clone = *dex_type;

                let task = async move {
                    match timeout(
                        Duration::from_secs(5),
                        dex_clone.supports_token_pair(&token_a_clone, &token_b_clone),
                    )
                    .await
                    {
                        Ok(Ok(supported)) => Some(supported),
                        Ok(Err(e)) => {
                            log::warn!("DEX {} support check error: {}", dex_type_clone, e);
                            None
                        }
                        Err(_) => {
                            log::warn!("DEX {} support check timeout", dex_type_clone);
                            None
                        }
                    }
                };
        ...
        // Return true if any DEX supports the pair
        for result in results {
