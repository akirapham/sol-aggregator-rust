...
pub async fn fetch_token(mint: &Pubkey, rpc_client: &Arc<RpcClient>) -> Result<Token, DexAggregatorError> {
    ...
    let mint_data = rpc_client.get_account_data(mint).await.map_err(|e| {
        DexAggregatorError::RpcError(format!("Failed to fetch account data for mint {}: {}", mint, e))
    })?;
